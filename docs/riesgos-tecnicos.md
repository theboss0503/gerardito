...
| :--- | :--- | :--- | :--- | :--- |
| **Volatilidad y Pérdida de Historial**<br>Actualmente los datos residen en `session_state`. Si la pestaña se cierra o recarga, se pierde todo el perfil del aspirante y la recomendación. *(Deuda Técnica)* | Datos | Alta | Alto | **Semana 2:** Integrar una base de datos relacional (SQLite) a través de un ORM para garantizar la persistencia de las sesiones. |
| **Fuerte Acoplamiento UI/Backend**<br>Toda la lógica de inferencia y la presentación visual conviven en un solo archivo, dificultando el mantenimiento y la escalabilidad. *(Deuda Técnica)* | Código | Alta | Alto | **Semana 2:** Desacoplar el sistema creando una API RESTful (FastAPI) y trasladar la interfaz visual a React.js. |
...
El mayor volumen de riesgo actual se concentra en el **Acoplamiento de Código** y la **Capa de Datos**, elementos típicos de un prototipo temprano (Proof of Concept). A nivel de infraestructura, la decisión de mantener Ollama en ejecución nativa y contenerizar únicamente el *Backend* y el *Frontend* es la estrategia de mitigación más inteligente para evitar los cuellos de botella clásicos de la virtualización de hardware gráfico.